...
@Component({
  selector: 'app-pcb-inspection',
  templateUrl: './pcb-inspection.component.html',
  styleUrls: ['./pcb-inspection.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class PCBInspectionComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, private location: Location, private logger: LoggerService, private pcbService: PCBInspectionService, private router: Router, private commonService: CommonService, private route: ActivatedRoute) {
  ...
  pcb_start() {
    ...
    Swal.fire({
      text: 'PCB Anomaly Usecase has started',
  ...
  pcb_stop() {
    ...
    Swal.fire({
      text: 'PCB Anomaly Usecase has stopped',
      showCancelButton: false,
      confirmButtonColor: '#3085d6',
      confirmButtonText: 'OK',
    });
  ...
  public pcb_timer() {
    ...
    this.pcbService.getPcbData()
      .subscribe(data => {
        ...
        if (this.pdata[this.i]["PcbSetType"] == "Damaged") {
          d += 1;
        }
        else if (this.pdata[this.i]["PcbSetType"] == "Defective") {
          g += 1;
        }
        else if (this.pdata[this.i]["PcbSetType"] == "Normal") {
          n += 1;
        }
        ...
        if (this.i == 0) {
          this.cam1 = this.pdata[this.i]["PcbProcessedImagePath"]
        }
        else if (this.i == 1) {
          this.cam1 = this.pdata[this.i]["PcbProcessedImagePath"]
          this.cam2 = this.pdata[this.i - 1]["PcbProcessedImagePath"]
          ...
          this.cam1 = this.pdata[this.i]["PcbProcessedImagePath"]
          this.cam2 = this.pdata[this.i - 1]["PcbProcessedImagePath"]
          this.cam3 = this.pdata[this.i - 2]["PcbProcessedImagePath"]
        ...
        this.serial1 = this.pdata[this.i]["PcbSetName"];
    ...
    onclick=\"open_timeline_popup(\''+ [pcbtype1, this.srl, this.srl_stat, this.srl_def, this.cam1, this.cam2, this.cam3, this.cam4] + '\')\">\
    ...
    <div class=\"card-body\" style="padding:0px;"><p style="font-size:14px;">Serial  # <a>'
        ...
        (error: any) => { this.logger.error(this.uname, "Pcb API is not Working. Error: " + JSON.stringify(error)); });
    ...
    if (/[P][C][B][_][0-9]/.test(this.search_string) == true) {
      ...
      if (this.search_string.slice(0, 4) == 'PCB_') {
        ...
        if (parseInt(this.pcb_id) > 0 && parseInt(this.pcb_id) <= (this.i) + 1) {
          ...
          this.logger.info(this.uname, "Pcb No. " + this.search_string + " is searched");
          let n = 0, g = 0, d = 0;
          ...
          this.cam1p = this.pdata[pid]["PcbProcessedImagePath"]
          this.cam2p = this.pdata[pid + 1]["PcbProcessedImagePath"]
          this.cam3p = this.pdata[pid + 2]["PcbProcessedImagePath"]
          this.cam4p = this.pdata[pid + 3]["PcbProcessedImagePath"]
          ...
          window.alert("PCB with Serial " + this.search_string + " is not available till now");
          this.pop_disp = "none";
        }
      }
      else {
        window.alert("Invalid Serial Number, Please Enter in correct Format e.g. PCB_<Number>");
        this.pop_disp = "none";
      }
    }
    else {
