...
import { ConvectionOvenService } from './convection-oven.service';

@Component({
  selector: 'app-convection-oven-rul',
  templateUrl: './convection-oven.component.html',
  styleUrls: ['./convection-oven.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ConvectionOvenComponent implements OnInit, OnDestroy {
  ...
  constructor(private location: Location, private logger: LoggerService, private convectionovenService: ConvectionOvenService, private router: Router, private route: ActivatedRoute, private boostService: CommonService) {
  ...
  ngOnInit(): void {
    ...
    this.logger.info(this.uname, this.uname + " Convection Oven Page has opened");
    this.logger.info(this.uname,this.uname+ " Convection Oven Page has opened");
  }


  checkAccessForUsecase(): any {
    if (!(this.ucases.indexOf('Convection Oven Health Profile') > -1)) {
  ...
  public millin() {
    // this.convectionovenService.getMilDataPageNumber().subscribe( 
    ...
    Swal.fire({
      text: 'Convection Oven Usecase has started',
      showCancelButton: false,
      confirmButtonColor: '#3085d6',
      confirmButtonText: 'OK',
    });
    this.logger.info(this.uname, "Convection Oven Usecase started");
  ...
  public milling_usecase() {
    ...
    if (this.in == 1) {
        ...
        this.predictions1 = new Chart('probabilities', {
            ...
            options: {
                title: { display: true, text: 'Convection Oven Health Indicator (%)', fontSize: 15 }, 
                scales:

                {
                    ...
                    yAxes: [{
                        scaleLabel: {
                            display: true,
                            labelString: "Convection Oven Health Indicator (%)",
                            fontColor: "black"
                        },
                        ticks: {
                            // autoSkip: true,
    ...
    this.convectionovenService.getMilData(this.in)
        .subscribe(data => 
        {
            ...
            if (this.mysv[0] > 0) {
                ...
                this.S1_AV.push(this.mysv[6]-23.3);
                this.S1_CP.push(this.mysv[7]);
                this.S1_CV.push(this.mysv[8]-23.3);
        ...
        (error: any)=>{ 
            console.log('Convection API is not working. Error: '+JSON.stringify(error));
            this.logger.error(this.uname,'Convection API is not working. Error: '+JSON.stringify(error));})
}
...
disp() {
    this.logger.info(this.uname,"Compare button of Convection Oven UseCase has been clicked");
...
public Stop() {
  ...
  Swal.fire({
    text: 'Convection Oven Usecase has stopped',
    showCancelButton: false,
    confirmButtonColor: '#3085d6',
    confirmButtonText: 'OK',
  });

    this.logger.info(this.uname,"Convection Oven Usecase stopped");
...
public info_pop() {
  ...
  swalWithBootstrapButtons.fire({
      html: '\
<img src=".\\assets\\img\\pop_milling.png" alt="Pop-up-Convection" style="width:966px;height: 500px;">\
',
      showClass: {
